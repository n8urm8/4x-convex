...
describe('System Mutations', () => {
  describe('discoverSystem', () => {
    ...
    test('should successfully discover an unexplored system', async () => {
      ...
      const { systemId, userId } = await t.run(async (ctx) => {
        ...
        const galaxyId = await ctx.db.insert('galaxies', {
          number: 1,
          groupId: 'test-galaxy-1'
        });
        const sectorId = await ctx.db.insert('galaxySectors', {
          galaxyId,
          galaxyNumber: 1,
          sectorX: 0,
          sectorY: 0
      ...
      const updatedSystem = await t.query(
        api.game.map.galaxyQueries.getStarSystemById,
        { systemId }
      );
    ...
    test('should return success:false if system already explored by another user', async () => {
      ...
      const result = await tAsSecondExplorer.mutation(
        api.game.map.systemMutations.discoverSystem,
        {
          systemId
        }
    ...
    test('should throw error if authenticated user has no record in users table', async () => {
      ...
      await expect(
        ...
        `User record not found for subject: ${nonExistentUserEmail}. Please ensure user exists.`
      );
    });
