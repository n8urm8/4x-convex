...
describe('Base Mutations', () => {
  ...
  describe('abandonBase', () => {
    ...
    test('should fail to abandon a base if not the owner', async () => {
        ...
        await expect(t.withIdentity({ subject: mockUserEmail2 }).mutation(api.game.bases.baseMutations.abandonBase, { baseId })).rejects.toThrow('User does not have permission to modify this base.');
    });
    ...
    test('should fail to abandon a base if not authenticated', async () => {
        ...
        await expect(t.mutation(api.game.bases.baseMutations.abandonBase, { baseId })).rejects.toThrow('User must be authenticated.');
    });
  });
