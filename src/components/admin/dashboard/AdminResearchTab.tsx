...
export function AdminResearchTab() {
  const researchDefinitions = useQuery(
    api.game.research.researchQueries.listResearchDefinitions
  );
  const [isCreateDialogOpen, setIsCreateDialogOpen] = useState(false);
  const [isEditDialogOpen, setIsEditDialogOpen] = useState(false);
  const [editingResearchDefinition, setEditingResearchDefinition] =
    useState<Doc<'researchDefinitions'> | null>(null);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [deletingResearchDefinition, setDeletingResearchDefinition] =
    useState<Doc<'researchDefinitions'> | null>(null);
  const deleteResearchMutation = useMutation(
    api.game.research.researchMutations.deleteResearchDefinition
  );
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.success(
        `Research '${deletingResearchDefinition.name}' deleted successfully!`
      );
  ...
  return (
    <div>
              ...
              <TableRow key={def._id}>
                ...
                <TableCell className="text-right">
                  <Button
                    ...
                    className="mr-2"
                    onClick={() => {
                      setEditingResearchDefinition(def);
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() => {
                      setDeletingResearchDefinition(def);
      ...
      <EditResearchDialog
        ...
        researchDefinition={editingResearchDefinition}
      />
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                <strong>{deletingResearchDefinition.name}</strong>".
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel
                onClick={() => setDeletingResearchDefinition(null)}
              >
                Cancel
              </AlertDialogCancel>
